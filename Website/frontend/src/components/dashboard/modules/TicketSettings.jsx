...
export default function TicketSettings({ config, channels, roles, categories, onSave, saving, onReset }) {
  ...
  return (
    <main className="p-4 lg:p-6 xl:p-8 max-w-[1200px] mx-auto flex flex-col gap-5">
      ...
      <div className="bg-neutral-900 rounded-2xl border border-neutral-800 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06),0_14px_34px_-20px_rgba(0,0,0,0.9)] flex items-center justify-between gap-4 p-5">
        <div className="flex flex-col min-w-0">
          <h2 className="text-[15px] font-semibold text-white truncate">Master Toggle</h2>
          <p className="text-[13px] text-neutral-400 mt-1 line-clamp-2">Enable or disable the ticket system on this server.</p>
        </div>
        <Toggle checked={ticketEnabled} onChange={() => setTicketEnabled(!ticketEnabled)} />
      </div>

      <div className={`mt-1 flex flex-col gap-6 transition-opacity duration-300 ${!ticketEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
