  ...
  return (
    <div className="pb-overview-container">
      ...
      <div className="pb-grid-bottom">
        ...
        <div className="lg:col-span-3 bg-[#111111] border border-neutral-800/80 rounded-2xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02),0_14px_34px_-20px_rgba(0,0,0,1)] flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800/80 bg-[#111111]">
            <div className="flex items-center gap-2.5">
              <ShieldAlert size={16} className="text-neutral-500" />
              <span className="text-sm font-semibold text-white">Recent mod activity</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-neutral-800/80 text-neutral-400 font-medium tabular-nums">{modActivity.length}</span>
            </div>
          ...
          <div className="flex flex-wrap gap-1.5 px-4 py-2.5 border-b border-neutral-800/60 bg-neutral-900/30">
            {Object.entries(modActivity.reduce((acc, a) => {
              ...
              return (
                <span key={type} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium ${isTimeout ? 'bg-amber-500/10 text-amber-400' : 'bg-neutral-800/80 text-neutral-400'}`}>
          ...
          <div className="flex flex-col flex-1 min-h-0 divide-y divide-neutral-800/60 overflow-y-auto scrollbar-thin bg-[#111111]">
            {modActivity.length === 0 ? (
              ...
              const isTimeout = (act.action || "").toLowerCase().includes("timeout") && !(act.action || "").toLowerCase().includes("un");
              const timeStr = act.timestamp ? Math.floor((Date.now()/1000 - act.timestamp) / 60) + "m ago" : "just now";
              return (
                <div key={i} className="flex items-center gap-2.5 px-4 py-2 hover:bg-neutral-800/30 transition-colors group" title={`${act.action} · ${act.reason} · by ${act.mod_name}`}>
                  <div className={`w-6 h-6 rounded-md ${isTimeout ? 'bg-amber-500/10' : 'bg-neutral-800/80'} flex items-center justify-center flex-shrink-0`}>
                    {isTimeout ? <Clock size={12} className="text-amber-400" /> : <Gavel size={12} className="text-neutral-400" />}
                  </div>
                  <div className="flex items-center gap-1.5 flex-1 min-w-0">
                    <span className="text-[13px] font-medium text-white truncate flex-shrink-0 max-w-[96px] sm:max-w-[140px]">{act.target_name}</span>
                    <span className={`text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded ${isTimeout ? 'bg-amber-500/10 text-amber-400' : 'bg-neutral-800/80 text-neutral-400'} flex-shrink-0`}>
                      {act.action}
                    </span>
                    <span className="text-[11px] text-neutral-500 truncate">{act.reason}</span>
                  </div>
                  <span className="text-[10px] text-neutral-500 tabular-nums flex-shrink-0">{timeStr}</span>
                </div>
              );
            })}
          </div>
