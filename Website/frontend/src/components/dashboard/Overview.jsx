  ...
  return (
    <div className="pb-overview-container">
      ...
      <div className="pb-grid-bottom">
        ...
        {/* Recent Mod Activity — exact 1:1 from provided HTML */}
        <div className="pb-col-mod bg-neutral-900 border border-neutral-800 rounded-2xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06),0_14px_34px_-20px_rgba(0,0,0,0.9)] flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800">
            <div className="flex items-center gap-2.5">
          ...
          {/* Summary pills — e.g. "1 untimeout", "1 timed out" */}
            ...
            {Object.entries(modActivity.reduce((acc, a) => {
              ...
              return (
                <span key={type} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium ${isTimeout ? 'bg-amber-500/10 text-amber-400' : 'bg-neutral-800 text-neutral-400'}`}>
                  {isTimeout ? <Clock className="w-2.5 h-2.5" /> : <Gavel className="w-2.5 h-2.5" />}
              ...
              return (
                <div key={i} className="flex items-center gap-2.5 px-4 py-2 hover:bg-neutral-800/30 transition-colors group" title={`${act.action} · ${act.reason} · by ${act.mod_name}`}>
                  <div className={`w-6 h-6 rounded-md ${isTimeout ? 'bg-amber-500/10' : 'bg-neutral-800'} flex items-center justify-center flex-shrink-0`}>
                    {isTimeout ? <Clock className="w-3.5 h-3.5 text-amber-400" /> : <Gavel className="w-3.5 h-3.5 text-neutral-400" />}
                  </div>
