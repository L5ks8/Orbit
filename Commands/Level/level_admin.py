...
class LevelAdminCog(commands.Cog):
    ...
    async def level_add(self, interaction: discord.Interaction, user: discord.Member, levels: int):
        if levels <= 0:
            return await interaction.response.send_message("Please provide a valid number of levels (> 0).", ephemeral=True)
        ...
        await interaction.response.send_message(f"✅ Successfully added **{levels}** levels to {user.mention}. They are now **Level {new_level}**.", ephemeral=True)
    ...
    async def level_remove(self, interaction: discord.Interaction, user: discord.Member, levels: int):
        ...
        await interaction.response.send_message(f"✅ Successfully removed **{levels}** levels from {user.mention}. They are now **Level {new_level}**.", ephemeral=True)
    ...
    async def level_set(self, interaction: discord.Interaction, user: discord.Member, level: int):
        ...
        await interaction.response.send_message(f"✅ Successfully set {user.mention} to **Level {level}**.", ephemeral=True)
    ...
    async def level_transfer(self, interaction: discord.Interaction, from_user: discord.Member, to_user: discord.Member):
        if from_user.id == to_user.id:
            return await interaction.response.send_message("You cannot transfer levels to the same user.", ephemeral=True)
        ...
        await interaction.response.send_message(f"✅ Successfully transferred all XP from {from_user.mention} to {to_user.mention}.", ephemeral=True)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status == 401 or resp.status == 403 or resp.status == 404:
                        return await interaction.followup.send("❌ Could not access your MEE6 leaderboard. Please ensure your MEE6 Leaderboard is set to **Public** in the MEE6 Dashboard, then try again.", ephemeral=True)
                    if resp.status != 200:
                        return await interaction.followup.send(f"❌ Failed to fetch data from MEE6. HTTP Status: {resp.status}", ephemeral=True)
                        
                    data = await resp.json()
                    players = data.get("players", [])
                    
                    if not players:
                        return await interaction.followup.send("❌ Found no players on the MEE6 leaderboard.", ephemeral=True)
                    ...
                    await interaction.followup.send(f"✅ Successfully imported **{imported_count}** users' levels from MEE6!", ephemeral=True)
                    
        except Exception as e:
            await interaction.followup.send(f"❌ An error occurred while importing: {e}", ephemeral=True)
