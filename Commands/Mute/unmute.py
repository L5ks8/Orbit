...
class UnmuteCommand(commands.Cog):
    ...
    @commands.hybrid_command(name="unmute", description="Removes the Muted role and Timeout from a user.")
    ...
    async def unmute(self, ctx: commands.Context, target: discord.Member, *, reason: str = "No reason provided"):
        ...
        was_muted = (role and role in target.roles) or target.is_timed_out()
        if not was_muted:
            return await ctx.send("This user is not currently muted or timed out.", ephemeral=True)

        try:
            try:
                if target.is_timed_out():
                    await target.timeout(None, reason=f"Unmuted by {ctx.author} | Reason: {reason}")
            except Exception:
                pass
            ...
            await ctx.send("I do not have permissions to remove the Muted role or Timeout.", ephemeral=True)
    ...
    async def unmute_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.send("You need Moderate Members / Manage Roles permission to unmute users.", ephemeral=True)
