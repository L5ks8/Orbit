...
class OwnerOverviewLayout(LayoutView):
    def __init__(self, owner: discord.abc.User):
        ...
        commands_str = (
            ...
            "**`-restart` / `-reboot` (Pre-Restart Safety Reboot)**\n"
            "> Backs up all persistent `Storage/` JSONs to the cloud backup channel and cleanly reboots the bot process.\n\n"
            "**`-reload [module/all]` (Pre-Reload Hot-Reloader)**\n"
            "> Pre-backs up storage to cloud and dynamically unloads/reloads command extensions without a full restart.\n\n"
            ...
            "**`-monitor` (System & Hardware Monitor)**\n"
            "> Tracks real-time process RAM (`MB`), virtual OS memory (`GB`), CPU load (`%`), shard info, and thread counts.\n\n"
