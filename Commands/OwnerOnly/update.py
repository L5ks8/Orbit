...
class UpdatePostModal(discord.ui.Modal, title="Post Orbit Changelog & Update"):
    title_input = discord.ui.TextInput(
        label="Update Title / Überschrift",
    ...
    version_input = discord.ui.TextInput(
        ...
        placeholder="e.g. v2.5.0 • Major Update",
        max_length=100,
        required=False
    )
    content_input = discord.ui.TextInput(
        label="Update Content / Beschreibung",
...
class UpdateLaunchLayout(LayoutView):
    def __init__(self, author_id: int):
        ...
        self.add_item(
            Container(
                TextDisplay(content="### 🌌 Orbit Update & Changelog Studio"),
                Separator(spacing=discord.SeparatorSpacing.small),
                TextDisplay(content=f"Click the button below to open your private update editor modal (`Überschrift & Inhalt`).\nWhen submitted, your announcement will be broadcasted directly to <#{UPDATE_CHANNEL_ID}>."),
                Separator(spacing=discord.SeparatorSpacing.small),
                ActionRow(btn_open)
            )
        )
