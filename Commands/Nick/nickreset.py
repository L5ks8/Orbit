...
@nick_group.command(name="reset", description="Resets a member's nickname back to their default username (`/nick reset`).")
...
class NickResetPrefixFallback(commands.Cog):
    ...
    @commands.command(name="nick reset", aliases=["nickreset"], hidden=True)
    @commands.has_permissions(manage_nicknames=True)
