...
from Commands.AutoMod._group import automod_group
...
@automod_group.command(name="setup", aliases=["panel"], description="Opens the interactive AutoMod & Server Protection Dashboard.")
...
class AutoModPrefixFallback(commands.Cog):
    ...
    @commands.command(name="am_panel", hidden=True)
    @commands.has_permissions(administrator=True)
    async def am_panel_prefix(self, ctx: commands.Context):
        await _do_automod_panel(ctx)
