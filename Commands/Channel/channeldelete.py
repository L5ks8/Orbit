...
@channel_group.command(name="delete", description="Deletes a channel (defaults to current channel if none specified).")
...
class ChannelDeleteCog(commands.Cog):
    ...
    @commands.command(name="deletechannel", aliases=["channeldelete", "delchannel", "delch"], hidden=True)
    @commands.has_permissions(manage_channels=True)
    ...
    async def delete_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send("Could not find that channel. Usage: `-deletechannel [#channel]`", ephemeral=True)
