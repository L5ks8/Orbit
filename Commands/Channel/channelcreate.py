...
from Commands.Channel._group import channel_group
...
@channel_group.command(name="create", description="Creates a new text or voice channel.")
...
class ChannelCreateCog(commands.Cog):
    ...
    @commands.command(name="createchannel", aliases=["channelcreate", "mkchannel", "mkch"], hidden=True)
    @commands.has_permissions(manage_channels=True)
    ...
    async def create_error(self, ctx: commands.Context, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send("Usage: `-createchannel <name> [text/voice] [#category]`", ephemeral=True)
        elif isinstance(error, commands.BadArgument):
            await ctx.send("Could not find that category. Usage: `-createchannel <name> [text/voice] [#category]`", ephemeral=True)
