...
@role_group.command(name="all", aliases=["roleall"], description="Assigns a role to every non-bot member on the server.")
...
@role_all_cmd.error
async def role_all_error(ctx: commands.Context, error):
...
class RoleAllFallback(commands.Cog):
    ...
    @commands.command(name="roleall", hidden=True)
    @commands.has_permissions(administrator=True)
    @commands.bot_has_permissions(manage_roles=True)
