...
@role_group.command(name="info", aliases=["roleinfo"], description="Displays detailed information about a specific server role.")
async def role_info_cmd(ctx: commands.Context, role: discord.Role):
    await _do_roleinfo(ctx, role)

@role_info_cmd.error
async def role_info_error(ctx: commands.Context, error):
