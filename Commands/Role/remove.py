...
async def _do_removerole(ctx: commands.Context, target: discord.Member, role: discord.Role, reason: str):
    ...
    try:
        ...
        kwargs = get_command_embed(ctx.guild.id, "role", msg_type="remove", member_mention=target.mention, member_id=target.id, role_mention=role.mention, role_id=role.id, role_color=role.color, reason=reason, author_mention=ctx.author.mention)
