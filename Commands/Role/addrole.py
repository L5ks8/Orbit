...
async def _do_addrole(ctx: commands.Context, target: discord.Member, role: discord.Role, reason: str):
    ...
    try:
        ...
        view = AddRoleSuccessLayout(target, role, reason, ctx.author)
...
@role_group.command(name="add", aliases=["addrole"], description="Assigns a role to a server member.")
...
@role_add_cmd.error
async def role_add_error(ctx: commands.Context, error):
